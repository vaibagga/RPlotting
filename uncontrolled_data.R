...
## loading the dataset
data_control1 <- read.xlsx('project.xlsx', 5)
data_control2 <- read.xlsx('project.xlsx', 7)
## create a new dataset
## taking data from sheet 5
combined_control <- data.frame(matrix(0, nrow = 103), stringsAsFactors = FALSE)
combined_control$Subject = data_control1$CONTROLLED.HBA1C..DEPRESSION.AND.ANXIETY.LEVELS.
...
combined_control$obBMI = data_control2$NA..5[6:105]
combined_control$SCS = data_control2$NA..6[6:105]
combined_control$matrix.1..nrow...103. <- NULL
## filling NAs

## age distribution in data
hist(as.numeric(as.character(combined_control$Age)), 
     main = 'Age Distribution in Controlled Data', 
     xlab = 'Age', 
     breaks = 20, 
     col = 'blue')
...
png(file = "Control Data Sex.jpg")

# Plot the chart.
pie(x, labels = piepercent, main = "Sex Distribution in Controlled Data",col = rainbow(length(x)))
...
plot(as.numeric((as.character(combined_control$Age))),
     ...
     main = "HBA1C levels vs Age in Controlled Data",
     xlab = "Age",
     ylab = "HBA1C levels",
     col = "red",
     pch = 4)
...
hist(as.numeric(as.character(males$HBA1C)), col = 'blue', breaks = 8, main = "HBA1C levels in Males in Controlled Data", xlab = "HBA1C level")
hist(as.numeric(as.character(females$HBA1C)), col = 'blue', breaks = 8, main = "HBA1C levels in Females in Controlled Data", xlab = "HBA1C level")

## Overall Distrinbution of HBA1C levels
hist(as.numeric(as.character(combined_control$HBA1C)), col = 'blue', breaks = 10, main = "Distribution of HBA1C lev
     els in Controlled Data", xlab = "HBA1C level")
...
hist(depLevel, breaks = 20, col = 'blue', main = 'Depression Levels in Controlled Data', xlab = 'Depression Level')
...
plot(depLevel, as.numeric(as.character(combined_control$HBA1C)),
     col = 'red',
     pch = 4,
     main = 'HBA1C Level vs Depression Level in Controlled Data',
...
hist(anxLevel, breaks = 20, col = 'blue', main = 'Anxiety Levels in Controlled Data', xlab = 'Anxiety Level')

plot(anxLevel, as.numeric(as.character(combined_control$HBA1C)),
     col = 'red',
     pch = 4,
     main = 'HBA1C level vs Anxity Level in Uncontrolled Data',
     xlab = 'Anxiety Level',
     ylab = 'HBA1C level')
...
BMILevel = c(as.numeric(as.character(combined_control$normBMI)),
             ...
             as.numeric(as.character(combined_control$obBMI)))
BMILevel[is.na(BMILevel)] = 0
BMILevel = BMILevel[1:100] + BMILevel[101:200] + BMILevel[201:300]

## BMI Level Distribution
hist(BMILevel, breaks = 20, col = 'blue', main = 'BMI in Controlled Data', xlab = 'BMI')

## sugar vs BMI
plot(BMILevel, as.numeric(as.character(combined_control$HBA1C)),
     col = 'red',
     pch = 4,
     main = 'HBA1C Level vs BMI in Controlled Data',
     xlab = 'BMI',
     ylab = 'HBA1C Level')
...
# Give the chart file a name.
png(file = "Control Data SCS-MSES-USES.jpg")
...
hist(as.numeric(as.character(scs$HBA1C)), col = 'blue', breaks = 8, main = "HBA1C levels in SCS in Controlled Data", xlab = "HBA1C level")
hist(as.numeric(as.character(uses$HBA1C)), col = 'blue', breaks = 8, main = "HBA1C levels in USES in Controlled Data", xlab = "HBA1C level")
